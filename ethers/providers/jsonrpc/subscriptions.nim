...
template convertErrorsToSubscriptionError(body) =
  convertErrorsTo(SubscriptionError):
    body
...
func start*(subscriptions: JsonRpcSubscriptions) =
  subscriptions.client.onProcessMessage = proc(
...
proc setMethodHandler(
    subscriptions: JsonRpcSubscriptions, `method`: string, handler: MethodHandler
) =
  subscriptions.methodHandlers[`method`] = handler

method subscribeBlocks*(
    subscriptions: JsonRpcSubscriptions, onBlock: BlockHandler
): Future[JsonNode] {.
    async: (raises: [SubscriptionError, CancelledError, RpcNetworkError]), base
.} =
  raiseAssert "not implemented"

method subscribeLogs*(
    subscriptions: JsonRpcSubscriptions, filter: EventFilter, onLog: LogHandler
): Future[JsonNode] {.
    async: (raises: [SubscriptionError, CancelledError, RpcNetworkError]), base
.} =
  raiseAssert "not implemented"
...
method unsubscribe*(
    subscriptions: JsonRpcSubscriptions, id: JsonNode
...
type WebSocketSubscriptions = ref object of JsonRpcSubscriptions
...
proc resubscribeWebsocketEventsOnTimeout*(
...
proc new*(
    _: type JsonRpcSubscriptions,
    client: RpcWebSocketClient,
    resubscribeInterval = WsResubscribe,
  ...
  if resubscribeInterval > 0:
    if resubscribeInterval >= 300:
      warn "Resubscription interval greater than 300 seconds is useless for hardhat workaround",
...
method subscribeBlocks*(
    subscriptions: WebSocketSubscriptions, onBlock: BlockHandler
): Future[JsonNode] {.
    async: (raises: [SubscriptionError, CancelledError, RpcNetworkError])
.} =
...
method subscribeLogs*(
    subscriptions: WebSocketSubscriptions, filter: EventFilter, onLog: LogHandler
): Future[JsonNode] {.
    async: (raises: [SubscriptionError, CancelledError, RpcNetworkError])
.} =
...
method unsubscribe*(
    subscriptions: WebSocketSubscriptions, id: JsonNode
) {.async: (raises: [CancelledError]).} =
  try:
    withLock(subscriptions):
      subscriptions.callbacks.del(id)
...
type PollingSubscriptions* = ref object of JsonRpcSubscriptions
  polling: Future[void]
...
method subscribeBlocks*(
    subscriptions: PollingSubscriptions, onBlock: BlockHandler
): Future[JsonNode] {.
    async: (raises: [SubscriptionError, CancelledError, RpcNetworkError])
.} =
...
method subscribeLogs*(
    subscriptions: PollingSubscriptions, filter: EventFilter, onLog: LogHandler
): Future[JsonNode] {.
    async: (raises: [SubscriptionError, CancelledError, RpcNetworkError])
.} =
  proc callback(id: JsonNode, argumentsResult: ?!JsonNode) =
...
method unsubscribe*(
    subscriptions: PollingSubscriptions, id: JsonNode
) {.async: (raises: [CancelledError]).} =
  try:
    subscriptions.logFilters.del(id)
    subscriptions.callbacks.del(id)
